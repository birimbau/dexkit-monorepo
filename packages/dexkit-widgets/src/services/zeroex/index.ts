...
export class ZeroExApiClient {
  ...
  async submitStatusGasless(
    ...
    { signal }: { signal?: AbortSignal }
  ): Promise<{ status: "confirmed" | "failed", transactions: { hash: string, timestamp: number }[], reason?: string }> {
    const resp = await this.axiosInstance.get(
      ZERO_EX_URL(this.chainId, this.siteId) + ZEROEX_GASLESS_STATUS_ENDPOINT,
      {
        params: tradeHash,
        signal,
      }

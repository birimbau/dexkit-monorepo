...
export async function getAppConfig(site?: string, appPage?: string): Promise<{ appConfig: AppConfig, appNFT?: AssetAPI | null, siteId?: number }> {
  /**/
  ...
  if (configResponse) {
    return { appConfig: JSON.parse(configResponse.config) as AppConfig, appNFT: configResponse.nft === undefined ? configResponse.nft : null, siteId: configResponse?.id };
  }

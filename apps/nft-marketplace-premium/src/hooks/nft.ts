...
import { getChainSlug, isAddressEqual } from '../utils/blockchain';
    ...
    async () => {
      ...
      try {
        assetApi = await getAssetDexKitApi({
          networkId: getChainSlug(chainId, NETWORKS) || '',
...
export function useAssetByApi({
  ...
  return useQuery(
    ...
    async () => {
      ...
      return await getAssetByApi({ chainId, contractAddress, tokenId, siteId });
    },
  );
}

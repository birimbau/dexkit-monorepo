...
export interface CountTokenDropsByGroupCardProps {
  title: React.ReactNode;
  filters: CountFilter;
  group: string;
  renderItem: (e: any, index: number) => React.ReactNode;
  renderGroup: (group: string, index: number) => React.ReactNode;
}

export default function CountTokenDropsByGroupCard({
  filters,
  renderItem,
  group,
  title,
  renderGroup,
}: CountTokenDropsByGroupCardProps) {
  const countEventsQuery = useCountDropTokenByGroup({
    filters,
    group,
  });
